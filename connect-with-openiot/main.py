...
DEVICE_ID='23355283'
API_KEY='pj4KUGgTVLFnz6Z0BY0bBxrWm=k='
...
def http_put_data(temp,hum):
    url='http://api.heclouds.com/devices/'+DEVICE_ID+'/datapoints'
    values={'datastreams':[{"id":"temp","datapoints":[{"value":temp}]},{"id":"hum","datapoints":[{"value":hum}]}]}
    jdata = json.dumps(values)
    try:
        urequests.post(url,data=jdata,headers={"api-key":API_KEY})
    except:
        print('系统错误，请重启继续使用')
    #return r
...
while True:
   ...
   hum =int(d.humidity())
   tm.number(temp)
   print("温度为%s 湿度为%s"%(temp,hum))
   http_put_data(temp,hum)
